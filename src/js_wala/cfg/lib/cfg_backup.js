...
define(function(require, exports) {
  ...
  function addEdges(src, dests) {
    sets.forEach(dests, function(dest) { 
        addEdge(src, dest); 
    });
  }
  ...
  function buildCFG(prog) {
      ...
      if (prog.body[0].type === 'ForStatement') {
          addEdge(fakeRoot, prog.body[0].init);
      } else if (prog.body[0].type === 'IfStatement') {
          addEdge(fakeRoot, prog.body[0].test);
      } else if (prog.body[0].type === 'WhileStatement') {
          addEdge(fakeRoot, prog.body[0].test);
      } else {
      ...
      var fakeEnd = new End();
      ast.setPosition(fakeEnd, ast.getPosition(prog));
      ast.setAttribute(prog, 'fakeEnd', fakeEnd);
  ...
  function buildFunctionCFG(fn, context) {
    ...
    if (fn.body.body[0].type === 'ForStatement') {
        addEdge(fakeRoot, fn.body.body[0].init);
    } else if (fn.body.body[0].type === 'IfStatement') {
        addEdge(fakeRoot, fn.body.body[0].test);
    } else if (fn.body.body[0].type === 'WhileStatement') {
        addEdge(fakeRoot, fn.body.body[0].test);
    } else {
        addEdge(fakeRoot, fn.body.body[0]);
    }
    var fakeEnd = new End();
    ast.setPosition(fakeEnd, ast.getPosition(fn));
    ast.setAttribute(fn, 'fakeEnd', fakeEnd);

    // build CFGs for all statements
    var accu = { exn: false, ret: false, brk: [] };
    context[context.length] = { type: 'function', node: fn };
    buildBlockCFG(fn.body.body, fakeEnd, context, accu);
  ...
  function buildBlockCFG(stmts, following, context, accu) {
      for(var i=0;i<stmts.length;++i) {
          var nextStmt = (i+1 < stmts.length) ? sets.singleton(stmts[i+1]) : following;
          //console.log(stmts[i].type + ", " + nextStmt.type);
          if (nextStmt.type === 'IfStatement') {
              nextStmt = nextStmt.test;
          }
          if (nextStmt.type === 'ForStatement') {
              nextStmt = nextStmt.init;
          }
          if (nextStmt.type === 'WhileStatement') {
              nextStmt = nextStmt.test;
          }

          buildStmtCFG(stmts[i], nextStmt, context, accu);

          // if statement is unreachable, introduce edge from fake root
          if(sets.size(ast.getAttribute(stmts[i], 'pred')) === 0) {
              if (stmts[i].type !== 'IfStatement' && stmts[i].type !== 'ForStatement' && stmts[i].type !== 'WhileStatement') {
  ...
  function buildStmtCFG(stmt, following, context, accu) {
      switch(stmt.type) {
          ...
          case 'ExpressionStatement':
              switch(stmt.expression.type) {
                  case 'CallExpression':
                      ...
                      addEdges(stmt, following);
                      if (stmt.expression.callee.type === 'FunctionExpression') {
          ...
          case 'IfStatement':
              addEdge(stmt.test, stmt.consequent.body[0]);
              buildStmtCFG(stmt.consequent, following, context, accu);
              if (stmt.alternate !== null) {
                  addEdge(stmt.test, stmt.alternate.body[0]);
                  buildStmtCFG(stmt.alternate, following, context, accu);
              } else {
                  addEdge(stmt.test, following);
              }
              break;

          case 'WhileStatement':
              addEdge(stmt.test, stmt.body.body[0]);
              addEdge(stmt.test, following);
              buildStmtCFG(stmt.body, stmt.test, context, accu);
              break;
          case 'ForInStatement':
              ...
              break;
          case 'ForStatement':
              addEdge(stmt.init, stmt.test);
              addEdge(stmt.test, stmt.body.body[0]);
              buildStmtCFG(stmt.body, stmt.update, context, accu);
              addEdge(stmt.update, stmt.test);
              addEdge(stmt.test, following);
              break;
